...
export function Investing() {
  ...
  useEffect(() => {
    async function fetchArticles() {
      try {
        ...
        // SMART FILTER: Catches investing, investment, or market
        const filtered = data.filter((article: Article) => {
          if (!article.category) return false;
          const cat = article.category.toLowerCase().trim();
          return ["investing", "investment", "market"].includes(cat);
        });
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      {articles.length === 0 ? (
        <p className="text-slate-500">No investment articles published yet.</p>
      ) : (
